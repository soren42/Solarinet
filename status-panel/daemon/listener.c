...
static int safeFile(const char *path){struct stat st;mode_t mode;if(path==NULL||path[0]=='\0'||stat(path,&st)!=0||!S_ISREG(st.st_mode))return -1;mode=st.st_mode&0777;if(mode!=0600&&mode!=0644)return -1;if(st.st_uid!=0&&st.st_uid!=geteuid())return -1;return 0;}
...
static void refuse(PanelListener *l,TlsPeer *peer){char who[512],message[768];X509 *cert=peer->ssl!=NULL?SSL_get1_peer_certificate(peer->ssl):NULL;identity(cert,who,sizeof(who));X509_free(cert);snprintf(message,sizeof(message),"TLS client refused peer=%.127s identity=%.500s",peer->peer,who);emit(l,message);}
