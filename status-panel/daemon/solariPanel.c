...
static void panelFrame(uint8_t type,const uint8_t *payload,size_t length,void *user){size_t i,n;char text[160];PanelLink *link=(PanelLink *)user;if(type==PANEL_FT_HELLO)logMessage("panel HELLO (%lu bytes)",(unsigned long)length);else if(type==PANEL_FT_EVENT&&length>=2u)logMessage("panel EVENT kind=%u arg=%u",payload[0],payload[1]);else if(type==PANEL_FT_LOG){n=length<sizeof(text)-1u?length:sizeof(text)-1u;for(i=0;i<n;++i)text[i]=(payload[i]>=32u&&payload[i]<127u)?(char)payload[i]:'.';text[n]='\0';logMessage("panel LOG %s",text);}else if(type==PANEL_FT_STATE&&link!=NULL)handleState(payload,length,link);else if(type==PANEL_FT_CONFIG&&link!=NULL)handleConfig(payload,length,link);}
