...
typedef struct { char apiBase[512],user[128],passFile[512],serialDev[512],listenAddr[256],tlsCert[512],tlsKey[512],clientCa[512],allowedSan[256],denylist[512];unsigned int pollSec,snapshotSec,listenPort; } Config;
...
typedef struct { uint8_t screenCfg[12], flags; } PanelScreenConfigReport;
typedef struct { CURL *curl; const Config *config; PanelStateReport lastState; uint32_t lastStatePostMs; uint64_t generation; PanelListener *listener; int stateSeen, haveState, withholdingLogged; } PanelLink;
...
static int loadConfig(const char *path, Config *config) { FILE *file;char line[1200];memset(config,0,sizeof(*config));config->pollSec=5u;config->snapshotSec=2u;strcpy(config->serialDev,"/dev/serial/by-id/*");file=fopen(path,"r");if(file==NULL){logMessage("cannot read config %s: %s",path,strerror(errno));return -1;}while(fgets(line,sizeof(line),file)!=NULL){char *key,*value,*equals;if(line[0]=='#'||line[0]=='\n')continue;equals=strchr(line,'=');if(equals==NULL)continue;*equals='\0';key=trim(line);value=trim(equals+1);if(strcmp(key,"apiBase")==0)snprintf(config->apiBase,sizeof(config->apiBase),"%s",value);else if(strcmp(key,"user")==0)snprintf(config->user,sizeof(config->user),"%s",value);else if(strcmp(key,"passFile")==0)snprintf(config->passFile,sizeof(config->passFile),"%s",value);else if(strcmp(key,"serialDev")==0)snprintf(config->serialDev,sizeof(config->serialDev),"%s",value);else if(strcmp(key,"listenAddr")==0)snprintf(config->listenAddr,sizeof(config->listenAddr),"%s",value);else if(strcmp(key,"tlsCert")==0)snprintf(config->tlsCert,sizeof(config->tlsCert),"%s",value);else if(strcmp(key,"tlsKey")==0)snprintf(config->tlsKey,sizeof(config->tlsKey),"%s",value);else if(strcmp(key,"clientCa")==0)snprintf(config->clientCa,sizeof(config->clientCa),"%s",value);else if(strcmp(key,"allowedSan")==0)snprintf(config->allowedSan,sizeof(config->allowedSan),"%s",value);else if(strcmp(key,"denylist")==0)snprintf(config->denylist,sizeof(config->denylist),"%s",value);else if(strcmp(key,"listen")==0){char *end;unsigned long port=strtoul(value,&end,10);if(*value=='\0'||*end!='\0'||port==0u||port>65535u){logMessage("invalid listen port");fclose(file);return -1;}config->listenPort=(unsigned int)port;}else if(strcmp(key,"pollSec")==0)config->pollSec=(unsigned int)strtoul(value,NULL,10);else if(strcmp(key,"snapshotSec")==0)config->snapshotSec=(unsigned int)strtoul(value,NULL,10);}fclose(file);if(config->apiBase[0]=='\0'||config->user[0]=='\0'||config->passFile[0]=='\0'||config->pollSec==0u||config->snapshotSec==0u){logMessage("config requires apiBase, user, passFile and positive intervals");return -1;}if(config->listenPort!=0u&&(config->tlsCert[0]=='\0'||config->tlsKey[0]=='\0'||config->clientCa[0]=='\0'||config->allowedSan[0]=='\0')){logMessage("listener requires tlsCert, tlsKey, clientCa and allowedSan");return -1;}return 0; }
...
/* Purpose: upload one decoded CONFIG report. Input: raw wire config/link. Output: zero for accepted API response. */
static int postConfig(PanelLink *link,const PanelScreenConfigReport *config){cJSON *json,*array,*entry;char *body,url[768];struct curl_slist *headers;HttpResponse response;CURLcode result;int i;if(link->curl==NULL)return -1;json=cJSON_CreateObject();array=cJSON_CreateArray();if(json==NULL||array==NULL||!cJSON_AddItemToObject(json,"screenCfg",array)){cJSON_Delete(json);cJSON_Delete(array);return -1;}for(i=0;i<12;++i){entry=cJSON_CreateObject();if(entry==NULL||!cJSON_AddNumberToObject(entry,"enabled",config->screenCfg[i]&1u)||!cJSON_AddNumberToObject(entry,"weightCode",(config->screenCfg[i]>>1)&7u)||!cJSON_AddItemToArray(array,entry)){cJSON_Delete(entry);cJSON_Delete(json);return -1;}}if(!cJSON_AddNumberToObject(json,"flags",config->flags)){cJSON_Delete(json);return -1;}body=cJSON_PrintUnformatted(json);cJSON_Delete(json);if(body==NULL)return -1;headers=curl_slist_append(NULL,"Content-Type: application/json");if(headers==NULL){cJSON_free(body);return -1;}snprintf(url,sizeof(url),"%s/api/panel/config",link->config->apiBase);curl_easy_setopt(link->curl,CURLOPT_HTTPHEADER,headers);result=request(link->curl,url,body,&response);curl_easy_setopt(link->curl,CURLOPT_HTTPHEADER,NULL);curl_slist_free_all(headers);cJSON_free(body);if(result!=CURLE_OK||response.status<200L||response.status>=300L||strncasecmp(trim(response.contentType),"application/json",16u)!=0){logMessage("panel config post failed (curl=%d HTTP=%ld)",(int)result,response.status);return -1;}return 0;}
...
static int parseSnapshot(const char *text,PanelSnapshot *snapshot) {
  ...
  /* CONTRACT-AW §3.1 (review M1): ingest the gear array so the v1-additive
   * trailer actually reaches the wire — PHP emits it, firmware renders it,
   * and without this hop A1 is NO DATA forever. Clamps are fail-dark:
   * unknown role drops the entry, unknown state reads as down. */
  {
    const cJSON *gearArr=cJSON_GetObjectItemCaseSensitive(data,"gear");
    if(cJSON_IsArray(gearArr)){
      int g=cJSON_GetArraySize(gearArr),o=0;
      if(g>(int)PANEL_MAX_GEAR)g=(int)PANEL_MAX_GEAR;
      for(i=0;i<g;++i){
        uint8_t role,st,rx,tx;
        item=cJSON_GetArrayItem(gearArr,i);
        if(!cJSON_IsObject(item))continue;
        role=byteValue(cJSON_GetObjectItemCaseSensitive(item,"role"),255);
        st=byteValue(cJSON_GetObjectItemCaseSensitive(item,"state"),0);
        rx=byteValue(cJSON_GetObjectItemCaseSensitive(item,"rxLevel"),0);
        tx=byteValue(cJSON_GetObjectItemCaseSensitive(item,"txLevel"),0);
        if(role>4u)continue;               /* unknown role: drop entry      */
        staged.gear[o].role=role;
        staged.gear[o].state=st>2u?0u:st;  /* unknown state: fail dark      */
        staged.gear[o].rxLevel=rx>7u?7u:rx;
        staged.gear[o].txLevel=tx>7u?7u:tx;
        ++o;
      }
      staged.gearCount=(uint8_t)o;
    }
  }
  cJSON_Delete(root);
...
/* Purpose: select the IPv4 address used by the default route without sending traffic. Input: output buffer. Output: zero for a non-wildcard address. */
static int defaultListenAddr(char *out,size_t cap){int fd;struct sockaddr_in remote,local;socklen_t length=sizeof(local);fd=socket(AF_INET,SOCK_DGRAM,0);if(fd<0)return -1;memset(&remote,0,sizeof(remote));remote.sin_family=AF_INET;remote.sin_port=htons(9u);if(inet_pton(AF_INET,"192.0.2.1",&remote.sin_addr)!=1||connect(fd,(struct sockaddr *)&remote,sizeof(remote))!=0||getsockname(fd,(struct sockaddr *)&local,&length)!=0||local.sin_addr.s_addr==htonl(INADDR_ANY)||inet_ntop(AF_INET,&local.sin_addr,out,(socklen_t)cap)==NULL){close(fd);return -1;}close(fd);return 0;}
...
typedef int (*FrameWriteFn)(void *context,const uint8_t *bytes,size_t length);
static int fdFrameWrite(void *context,const uint8_t *bytes,size_t length){return writeAll(*(int *)context,bytes,length);}
static int tlsFrameWrite(void *context,const uint8_t *bytes,size_t length){return panelListenerWrite((PanelListener *)context,bytes,length);}
...
static void handleState(const uint8_t *payload,size_t length,PanelLink *link){PanelStateReport state;uint64_t generation=link->generation;int changed;memset(&state,0,sizeof(state));if(panelDecodeState(payload,length,&state.theme,&state.screen,&state.brightnessPct,&state.autoBright,&state.sleeping,&state.alarmArmed,&state.alarmAcked,&state.dwellSec,&state.ackedEpisodeId,&state.lastCmdId)!=0){logMessage("malformed panel STATE (%lu bytes)",(unsigned long)length);return;}link->stateSeen=1;changed=!link->haveState||memcmp(&state,&link->lastState,sizeof(state))!=0;if(changed)logMessage("panel STATE theme=%u screen=%u brightness=%u auto=%u sleeping=%u armed=%u acked=%u dwell=%u episode=%lu lastCmdId=%lu",state.theme,state.screen,state.brightnessPct,state.autoBright,state.sleeping,state.alarmArmed,state.alarmAcked,state.dwellSec,(unsigned long)state.ackedEpisodeId,(unsigned long)state.lastCmdId);if(changed||(uint32_t)(nowMs()-link->lastStatePostMs)>=30000u){if((link->listener==NULL||generation==panelListenerGeneration(link->listener))&&postState(link,&state)==0&&(link->listener==NULL||generation==panelListenerGeneration(link->listener))){link->lastState=state;link->haveState=1;link->lastStatePostMs=nowMs();}}}
/* Purpose: decode and upload the CONTRACT-AW §3.3 CONFIG payload. Input: 16-byte frame payload/link. Output: one API post or malformed-frame log. */
static void handleConfig(const uint8_t *payload,size_t length,PanelLink *link){PanelScreenConfigReport config;/* Review S1: use the shared codec, not a hand-rolled memcpy — panelDecodeConfig
 * enforces the D3 reserved-bit checks that the mutation round proved load-bearing. */
if(panelDecodeConfig(payload,length,config.screenCfg,&config.flags)!=0){logMessage("malformed panel CONFIG (%lu bytes)",(unsigned long)length);return;}if(postConfig(link,&config)==0)logMessage("panel CONFIG flags=%u",config.flags);}
...
static void panelFrame(uint8_t type,const uint8_t *payload,size_t length,void *user){size_t i,n;char text[160];PanelLink *link=(PanelLink *)user;if(type==PANEL_FT_HELLO)logMessage("panel HELLO (%lu bytes)",(unsigned long)length);else if(type==PANEL_FT_EVENT&&length>=2u){uint8_t coverage;uint32_t serverEpisode,powerEpisode;if(payload[0]==PANEL_EV_ACK&&panelDecodeAckEvent(payload,length,&coverage,&serverEpisode,&powerEpisode)==0)logMessage("panel EVENT ack coverage=%u serverEpisode=%lu powerEpisode=%lu",coverage,(unsigned long)serverEpisode,(unsigned long)powerEpisode);else logMessage("panel EVENT kind=%u arg=%u",payload[0],payload[1]);}else if(type==PANEL_FT_LOG){n=length<sizeof(text)-1u?length:sizeof(text)-1u;for(i=0;i<n;++i)text[i]=(payload[i]>=32u&&payload[i]<127u)?(char)payload[i]:'.';text[n]='\0';logMessage("panel LOG %s",text);}else if(type==PANEL_FT_STATE&&link!=NULL)handleState(payload,length,link);else if(type==PANEL_FT_CONFIG&&link!=NULL)handleConfig(payload,length,link);}
static void listenerFrame(uint8_t type,const uint8_t *payload,size_t length,uint64_t generation,void *user){PanelLink *link=user;if(link->generation!=generation){link->generation=generation;link->stateSeen=0;link->haveState=0;link->withholdingLogged=0;}panelFrame(type,payload,length,link);}
static void listenerLog(const char *message,void *user){(void)user;logMessage("%s",message);}
...
static int sendFrameTo(FrameWriteFn writer,void *context,const PanelSnapshot *snapshot,int sendSnapshot){uint8_t payload[PANEL_MAX_PAYLOAD],frame[PANEL_HDR_SIZE+PANEL_MAX_PAYLOAD+PANEL_CRC_SIZE];size_t payloadLength=0u,frameLength;if(sendSnapshot)payloadLength=panelEncodeSnapshot(snapshot,payload,sizeof(payload));frameLength=panelEncodeFrame(sendSnapshot?PANEL_FT_SNAPSHOT:PANEL_FT_PING,sendSnapshot?payload:NULL,payloadLength,frame,sizeof(frame));return frameLength==0u?-1:writer(context,frame,frameLength);}
static int sendFrame(int fd,const PanelSnapshot *snapshot,int sendSnapshot){return sendFrameTo(fdFrameWrite,&fd,snapshot,sendSnapshot);}
...
static int runDaemon(const Config *config,const char *password){
  CURL *curl=NULL; PanelSnapshot latest; PanelParser parser; PanelLink serialLink,tlsLink;
  PanelListener *listener=NULL; PanelListenerConfig listenerConfig; char resolvedAddr[INET_ADDRSTRLEN];
  int haveLatest=0,fd=-1,tlsWasConnected=0;
  uint16_t tlsSeq=0u;
  uint32_t now=nowMs(),nextPoll=now,nextFrame=now,nextTlsFrame=now,nextSerial=now,authDelay=1u,transientDelay=1u,serialDelay=1u;
  int lastPollOk=0; uint32_t nextGlance=now;   /* at-a-glance matrix feed */
  memset(&latest,0,sizeof(latest));memset(&serialLink,0,sizeof(serialLink));memset(&tlsLink,0,sizeof(tlsLink));serialLink.config=config;tlsLink.config=config;panelParserInit(&parser);
  if(config->listenPort!=0u){const char *address=config->listenAddr;if(address[0]=='\0'){if(defaultListenAddr(resolvedAddr,sizeof(resolvedAddr))!=0){logMessage("cannot determine a safe default LAN listenAddr; configure listenAddr explicitly");return 1;}address=resolvedAddr;logMessage("listenAddr not set; selected default-route address %s",address);}memset(&listenerConfig,0,sizeof(listenerConfig));listenerConfig.address=address;listenerConfig.port=config->listenPort;listenerConfig.certificate=config->tlsCert;listenerConfig.privateKey=config->tlsKey;listenerConfig.clientCa=config->clientCa;listenerConfig.allowedSan=config->allowedSan;listenerConfig.denylist=config->denylist;listener=panelListenerCreate(&listenerConfig,listenerFrame,&tlsLink,listenerLog,NULL);if(listener==NULL)return 1;tlsLink.listener=listener;logMessage("TLS listener ready on %s:%u",address,config->listenPort);}
  while(running){
    now=nowMs();serialLink.curl=curl;tlsLink.curl=curl;
    if(listener!=NULL){uint64_t before=panelListenerGeneration(listener);panelListenerService(listener,now);if(panelListenerGeneration(listener)!=before||tlsLink.generation!=panelListenerGeneration(listener)){tlsLink.generation=panelListenerGeneration(listener);tlsLink.stateSeen=0;tlsLink.haveState=0;tlsLink.withholdingLogged=0;tlsSeq=0u;nextTlsFrame=now;}if(tlsWasConnected&&!panelListenerConnected(listener)){tlsLink.stateSeen=0;tlsLink.haveState=0;tlsLink.withholdingLogged=0;}tlsWasConnected=panelListenerConnected(listener);}
    if(curl==NULL&&(int32_t)(now-nextPoll)>=0){curl=login(config,password);serialLink.curl=curl;tlsLink.curl=curl;if(curl==NULL){nextPoll=now+authDelay*1000u;authDelay=authDelay<60u?authDelay*2u:60u;continue;}else{authDelay=1u;nextPoll=now;}}
    if(curl!=NULL&&(int32_t)(now-nextPoll)>=0){
      ...
      if(result==CURLE_OK&&response.status==401L){curl_easy_cleanup(curl);curl=login(config,password);serialLink.curl=curl;tlsLink.curl=curl;if(curl!=NULL)result=request(curl,url,NULL,&response);}
      if(result==CURLE_OK&&response.status>=200L&&response.status<300L&&strncasecmp(trim(response.contentType),"application/json",16u)==0&&parseSnapshot(response.data,&latest)==0){
        ...
        if(fd>=0){
          ...
          if(sendFrame(fd,&latest,1)!=0||forwardCommands(fd,response.data,&serialLink)!=0){logMessage("serial write failure; reopening");close(fd);fd=-1;nextSerial=now+serialDelay*1000u;serialDelay=serialDelay<30u?serialDelay*2u:30u;}else nextFrame=now+config->snapshotSec*1000u;
        }
        if(listener!=NULL&&panelListenerConnected(listener)){PanelSnapshot wire=latest;wire.seq=++tlsSeq;if(sendFrameTo(tlsFrameWrite,listener,&wire,1)!=0||forwardCommandsTo(tlsFrameWrite,listener,response.data,&tlsLink)!=0)logMessage("TLS panel write failure; closing session");else nextTlsFrame=now+config->snapshotSec*1000u;}
      }else if(response.status==401L){curl_easy_cleanup(curl);curl=NULL;serialLink.curl=NULL;tlsLink.curl=NULL;nextPoll=now+authDelay*1000u;authDelay=authDelay<60u?authDelay*2u:60u;}
    ...
    if(config->serialDev[0]!='\0'&&fd<0&&(int32_t)(now-nextSerial)>=0){fd=openSerial(config->serialDev);if(fd<0){nextSerial=now+serialDelay*1000u;serialDelay=serialDelay<30u?serialDelay*2u:30u;}else{serialDelay=1u;nextSerial=now;panelParserInit(&parser);serialLink.stateSeen=0;serialLink.haveState=0;serialLink.withholdingLogged=0;nextFrame=now;
    ...
    if(fd>=0){
      if(readSerial(fd,&parser,&serialLink)!=0){logMessage("serial read failure; reopening");close(fd);fd=-1;nextSerial=now+serialDelay*1000u;serialDelay=serialDelay<30u?serialDelay*2u:30u;continue;}
    ...
    if(listener!=NULL&&panelListenerConnected(listener)&&(int32_t)(now-nextTlsFrame)>=0){PanelSnapshot wire=latest;if(haveLatest){int64_t age=(int64_t)time(NULL)-(int64_t)wire.ts;wire.dataStale=(uint8_t)(wire.dataStale!=0u||age>35);wire.seq=++tlsSeq;}if(sendFrameTo(tlsFrameWrite,listener,&wire,haveLatest)!=0)logMessage("TLS panel write failure; closing session");nextTlsFrame=now+config->snapshotSec*1000u;}
    /* Feed the on-board glance matrix every 2 s (cosmetic, fail-silent). */
    if((int32_t)(now-nextGlance)>=0){PanelLink *glanceLink=fd>=0?&serialLink:&tlsLink;glanceWrite(&latest,haveLatest,lastPollOk,glanceLink,fd>=0);nextGlance=now+2000u;}
    sleepMs(20L);
  }
  if(fd>=0) close(fd);
...
int panelForwardCommandsTlsForTest(PanelListener *listener,const char *text,int stateSeen){PanelLink link;memset(&link,0,sizeof(link));link.stateSeen=stateSeen;return forwardCommandsTo(tlsFrameWrite,listener,text,&link);}
#else
