...
1. **Bidirectional SoR sync — LIVE + closed** (`deploy/sorsync/` + `dashboard/api/lib/Sor.php`): operator create/alter/remove in the dashboard → **writes the SoR** → CDC outbox → `sor.events` → DNS auto-rendered/reloaded; and detected+adopted host → SoR → onward to DNS. Verified end-to-end both directions (insert/update/delete). Render targets: BIND (xenon), **AD `akoria.org`** (`sor_apply_ad` on radium, safe state-file deletes), **Pi-hole** (`sor_apply_pihole` on xenon → helium/mercury) — all live + verified. Remaining extensions: config as a render target; Pi-hole CNAMEs; monotonic zone serial for secondary AXFR; outbox pruning; retire the now-redundant hourly `akoria-dns-reconcile`.
